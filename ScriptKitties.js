...
var minorOptions = [
    ['Auto Observe', 'observe', false],
    ['Auto Feed Elders', 'feedElders', false],
    ['Auto Feed Elders', 'feedElders', false],
    ['Auto Promote Leader', 'promote', false],
    ['Unicorn Ivory Optimization', 'unicornIvory', false],
];
...
$("#SK_optionSelect").hide(); // only way I can find to have display:grid but start hidden
...
function generateMinorOptionsMenu() {
    ...
    for (var i=0; i<minorOptions.length; i++) {
        [label,id] = minorOptions[i];
        menu += `<input type="checkbox" id="SK_${id}" onchange="verifyElementSelected(minorOptions,\'${id}\',this.checked)">`;
        menu += `<label style="padding-left:10px;" for="SK_${id}">${label}</label><br>`;
    }
    menu += '</div>';
    return menu;
}
...
function generateBuildingPane(groups, elementsName) {
    ...
    for (var i = 0; i < groups.length; i++)  {
        ...
        menu += `<input type="checkbox" id="SK_${lab}Checker" onclick="selectChildren('SK_${lab}Checker','SK_${lab}Check');">`;
